...
const controller = {
  ...
  create: (req, resp) => {
    ...
    try {
      ...
      if (product.gallery.length > 0) {
        const newGallery = product.gallery.map((el) => {
          let pic = picture.getPicture(el);
          console.log(pic);
          return pic;
        });
  ...
  search: (req, resp) => {
    ...
    try {
      ...
      const info = data.filter((el) => {
        ...
        if (el.description) {
          if (el.description.includes(keyword)) ret = el;
        }
        if (el.category) {
          if (el.category.includes(keyword)) ret = el;
        }
        return ret;
      });
  ...
  chekData: (req, resp, next) => {
    ...
    if (!title || !price) {
      resp
        .status(400)
        .json({ mssage: "Los valores  title y precio son obligatorios" });
    } else {
      if (Number(price) < 0) {
