...
const pictureController = {
  ...
  getPictureID: (req, res) => {
    let datosPictures = JSON.parse(
      fs.readFileSync(path.resolve(__dirname, "../data/pictures.json")),
      "utf-8"
    );
  ...
  editPicture: (req, res) => {
    ...
    datosPictures.forEach((ele) => {
      if (ele.id == id) {
        if (ele.url) ele.url = req.body.url;
        else return res.status(400).json("debe ingresar una url");
        if (ele.descripcion) ele.descripcion = req.body.descripcion;
        encontre = true;
      }
    });
    if (!encontre) {
